...
def test_allocating_to_a_batch_reduces_the_available_quantity():
    (
        batch,
        order_line,
    ) = make_batch_and_order_line("SMALL-TABLE", 20, 2)
...
def test_can_allocate_if_available_equal_to_required():
    (
        batch,
        order_line,
    ) = make_batch_and_order_line("SMALL-TABLE", 5, 5)
...
def test_cannot_allocate_if_available_smaller_than_required():
    (
        batch,
        order_line,
    ) = make_batch_and_order_line("SMALL-TABLE", 2, 20)
...
def test_allocation_is_idempotent():
    (
        batch,
        line,
    ) = make_batch_and_order_line("ANGULAR-DESK", 20, 2)
    batch.allocate(line)
    batch.allocate(line)
    assert batch.available_quantity == 18
